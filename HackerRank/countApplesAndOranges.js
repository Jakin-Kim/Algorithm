...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    ...
    let applesInSam = apples.map(apple => a + apple).filter(apple => (apple >= s && apple <= t));
    let orangesInSam = oranges.map(orange => b + orange).filter(orange => (orange >= s && orange <= t));
    
    console.log(applesInSam.length);
    console.log(orangesInSam.length);
}
