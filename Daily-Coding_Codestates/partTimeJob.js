...
function partTimeJob(k) {
    ...
    //k = 4100 , 4972, 5543, 6000
    let count = 0;
    for (let i = coins.length - 1; i >= 0; i--) {
      
      if (k % coins[i] !== 0) { // 나누어 떨어지지 않으면
        count += parseInt(k / coins[i]); // 몫을 넣어주고
        k = k % coins[i]; // 나머지는 재할당해주기
        
      }
  
      if (k % coins[i] === 0) { // 나누어 떨어지면
        return count += parseInt(k / coins[i]); // 몫을 리턴
      } 
    }
...
// NaN이 나와서 당황한 이유
// 초기값과 배열의 길이를 정확하게 확인하지 않았기 때문에
